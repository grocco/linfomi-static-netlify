      ...
      <div className="aside-left">
        ...
        <div className="member-details">
          <div className="board-of-directors">
            {member.data["board-of-directors"] ? i18n.pages.council.boardOfDirectors[language] : ""}
          </div>
          <div className="title">
            {member.data.surname} {member.data.name}
          </div>
...
class MemberListItem extends React.Component {
  ...
  render() {
    ...
    const inner = (
      <div
        className={`member-list-item ${selected ? "selected" : ""} ${
          scientificCommittee || member.data["not-anymore"] ? "simple" : ""
        }`}
      >
        <div className="pic-name-role">
          <div ref={el => (this.lift = el)} className="lift-container">
            <div
              className="round member-pic"
              style={{
...
class Council extends React.Component {
  ...
  renderList() {
    ...
    const { members, language, scientificCommittee, scMembers, scientific, exmembers } = this.props;
    const council =
      !scientific && !exmembers ? (
        <div>
          <div className="row-size-text">
            <span>{_l(i18n.pages.council.current)}</span>
          </div>
          <div className="members">
            {members &&
              members
                ...
                .sort(
                  (el1, el2) =>
                    compareByField("order", el => el.data)(el1, el2) ||
                    compareByField("surname", el => el.data)(el1, el2)
                )
                .map((member, idx) => (
                  <MemberListItem
                    history={this.props.history}
                    key={member.data.slug}
                    member={member}
                    language={language}
                    selected={
                      this.props.history.location.state &&
                      (this.props.history.location.state.memberSlug === member.data.slug ||
                        (idx === 0 && this.props.history.location.state.memberSlug === "president"))
                    }
                  />
                ))}
          </div>
        ...
        <div className="other-members">
          {exmembers && (
            <div className="members" style={{ flex: 2 }}>
              <div className="row-size-text">
                <span>{_l(i18n.pages.council.previous)}</span>
              </div>
              {members &&
                members
                  .filter(member => member.data["not-anymore"])
          ...
          {scientific && (
            <div className="members" style={{ flex: 3 }}>
              <div className="row-size-text">
                <span>{_l(i18n.pages.council.scientific)}</span>
              </div>
              {scMembers &&
                scMembers
                  .sort(compareByField("surname", el => el.data))
                  ...
                  .map(member => (
                    <div className="scientific-council-member" key={member.data.slug}>
                      <span>
                        {member.data.surname} {member.data.name}{" "}
                        {l(member.data, "role", language) && `(${l(member.data, "role", language)})`}
                      </span>
                      <span>
                        {member.data.institute}
                        {member.data.city && `, ${member.data.city}`}
    ...
    const sc = (
      <div>
        <div className="row-size-text">
          <span>{l(i18n.pages.council.current)}</span>
        </div>
        <div className="members">
          {members &&
            members
              ...
              .sort(
                (el1, el2) =>
                  compareByField("order", el => el.data)(el1, el2) || compareByField("surname", el => el.data)(el1, el2)
              )
              .map((member, idx) => (
                <MemberListItem
                  history={this.props.history}
  ...
  render() {
    ...
    let member = null;
    if (history.location.state && history.location.state.memberSlug === "president")
      member = members.sort(compareByField("order", el => el.data))[0];
    else
      member = members.find(member => history.location.state && member.data.slug === history.location.state.memberSlug);
    return (
